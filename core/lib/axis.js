...
class AxisViewer {
    ...
    constructor(scene, size) {
        this._scene = scene;
        this._axis = new AxesViewer(scene, size);
    ...
    _make_axis_label(text, color, size) {
        var dynamicTexture = new DynamicTexture("DynamicTexture", 50, this._scene, true);
        dynamicTexture.hasAlpha = true;
        dynamicTexture.drawText(text, 5, 40, "bold 36px Arial", color, "transparent", true);
        var plane = MeshBuilder.CreatePlane("TextPlane", { size: size }, this._scene);
        let material = new StandardMaterial("TextPlaneMaterial", this._scene);
...
class AxisHelper {
    ...
    constructor(engine, camera) {
        ...
        var cameraGizmo = new ArcRotateCamera("cam1", 2.0, Math.PI / 2, 5, Vector3.Zero(), scene);
