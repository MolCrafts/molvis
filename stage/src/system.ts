...
export class System {
  ...
  private async _navigateTo(rawIndex: number): Promise<boolean> {
    ...
    // Warm playhead neighbors so continuous play hits the LRU. Forward
    // neighbor first (playback direction), then reverse for scrub.
    this._trajectory.prefetch([index + 1, index - 1]);
